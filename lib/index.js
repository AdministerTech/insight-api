...
var MasternodeRepository = require('../repositories/MasternodeRepository');;
...
var InsightAPI = function(options) {
  ...
  this.masternodeRepository = new MasternodeRepository();
  ...
  this.masternodeService = new MasternodeService({node: this.node, masternodeRepository: self.masternodeRepository});
...
InsightAPI.prototype.start = function(callback) {
  ...
  return async.waterfall([function (callback) {
      ...
      return self.masternodeService.start(function (err) {
         return callback(err);
      });
  }], function (err) {
...
InsightAPI.prototype.setupRoutes = function(app) {
  ...
  //Masternodes
  var mnOptions = {node: this.node, masternodeService: this.masternodeService, masternodeRepository: this.masternodeRepository};
  var masternodes = new MasternodesController(mnOptions);
  app.get('/masternodes/raw', this.cacheShort(), masternodes.raw.bind(masternodes));
  app.get('/masternodes/list', this.cacheShort(), masternodes.list.bind(masternodes));
  app.get('/masternodes/stats', this.cacheShort(), masternodes.stats.bind(masternodes));
