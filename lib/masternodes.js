...
function MasternodeController(options) {
	this.node = options.node;
	this.masternodeService = options.masternodeService;
	this.masternodeRepository = options.masternodeRepository;
	this.common = new Common({log: this.node.log});
}

MasternodeController.prototype.stats = function(req, res) {
	// construct ninja api here
	let self = this;
	res.json(self.masternodeRepository.mnCache);
};

MasternodeController.prototype.list = function(req, res) {
	let self = this;
	this.masternodeService.getMNList(function(err, result) {
		if (err) {
...
MasternodeController.prototype.raw = function(req, res) {
	this.node.services.ravend.getMNListFull(function(err, result){
		var MNList = result.result || {};
		if (err) {
			return self.common.handleErrors(err, res);
		}
		res.json(MNList);
